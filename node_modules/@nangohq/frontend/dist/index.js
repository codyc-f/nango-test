...
export default class Nango {
    ...
    convertCredentialsToConfig(credentials) {
        ...
        if ('pat_name' in credentials && 'pat_secret' in credentials) {
            ...
            return { params: tableauCredentials };
        }
        return { params };
    }
    /**
    ...
    async customAuth(providerConfigKey, connectionId, connectionConfigWithCredentials, connectionConfig) {
        ...
        if ('token_id' in credentials && 'token_secret' in credentials) {
            const tbaCredentials = credentials;
            const url = this.hostBaseUrl + `/auth/tba/${providerConfigKey}${this.toQueryString(connectionId, connectionConfig)}`;
            const res = await fetch(url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
        ...
        if ('pat_name' in credentials && 'pat_secret' in credentials) {
            const tableauCredentials = credentials;
            const url = this.hostBaseUrl + `/auth/tableau/${providerConfigKey}${this.toQueryString(connectionId, connectionConfig)}`;
            const res = await fetch(url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
